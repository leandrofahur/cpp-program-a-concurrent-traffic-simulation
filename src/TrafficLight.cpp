...
void TrafficLight::cycleThroughPhases()
{
    ...
    // Distribtuions
    //
    std::uniform_real_distribution<int> dist(4000, 6000);
    ...
    while(true)
    {
        ...
        int elapsed_seconds = std::chrono::duration_cast<std::chrono::milliseconds>
                             (std::chrono::system_clock::now()-timeNow).count();

        if(elapsed_seconds >= dist(e2))
        {
            ...
            std::future<TrafficLightPhase> futVar = std::async(&MessageQueue<TrafficLightPhase>::send, std::move(_currentPhase));
            futVar.wait();
        }
